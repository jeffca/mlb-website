...
import TeamsStatsTable from './tables/TeamsStatsTable.vue';
...
const fetchStat = async (stat) => {
  const res = await fetch(`http://localhost:3010/teams/today/batting/${stat}/last/${dateRange.value}`);
  return res.json();
};
...
<template>
  <div>
    ...
    <TeamsStatsTable :stats="hits" :title="'hits'" />
    <TeamsStatsTable :stats="strikeouts" :title="'strikeouts'" />
    <TeamsStatsTable :stats="walks" :title="'walks'" />
    <TeamsStatsTable :stats="rbis" :title="'rbis'" />
  </div>
</template>
